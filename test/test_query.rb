...
# Unit tests for Ssot::Query module
...
class TestQueryRun < Minitest::Test
  def test_run_help
    out, err = capture_io { Ssot::Query.run(['help']) }
    assert_match(/SSoT Database Query Tool/, out)
    assert_match(/list-packages/, out)
  end

  def test_run_help_is_default
    out, err = capture_io { Ssot::Query.run([]) }
    assert_match(/SSoT Database Query Tool/, out)
  end

  def test_run_aliases
    out, err = capture_io { Ssot::Query.run(['lp']) }
    assert_match(/Platforms/, out)

    out, err = capture_io { Ssot::Query.run(['h']) }
    assert_match(/SSoT Database Query Tool/, out)
  end
end
...
class TestQueryPrintHelp < Minitest::Test
  def test_print_help_shows_commands
    out, err = capture_io { Ssot::Query.print_help }
    assert_match(/list-packages/, out)
...
class TestQueryListPlatforms < Minitest::Test
  def test_list_platforms_shows_registry
    out, err = capture_io { Ssot::Query.list_platforms }
    assert_match(/Platforms/, out)
...
class TestQuerySearch < Minitest::Test
  def test_search_no_results
    out, err = capture_io { Ssot::Query.search('xyznonexistent12345') }
...
class TestQueryShowProvides < Minitest::Test
  def test_show_provides_no_providers
    out, err = capture_io { Ssot::Query.show_provides('xyznonexistent') }
