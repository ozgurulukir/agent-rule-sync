...
class TestCompareVersions < Minitest::Test
  ...
  def test_year_based_versioning
    ...
    assert_equal 0, Ssot::Lib::Common.compare_versions('2026.05', '2026.05')
  end

  def test_single_segment_versions
    assert_equal 1, Ssot::Lib::Common.compare_versions('2', '1')
    assert_equal 0, Ssot::Lib::Common.compare_versions('1', '1')
    assert_equal -1, Ssot::Lib::Common.compare_versions('1', '2')
  end
  ...
  def test_three_segment_numeric_comparison
    ...
    assert_equal -1, Ssot::Lib::Common.compare_versions('1.9.0', '1.10.0')
  end

  def test_epoch_zero_defaults
    # epoch=0 for both is default — should compare by pkgver only
    assert_equal 0, Ssot::Lib::Common.compare_versions('1.0.0', '1.0.0', epoch1: 0, epoch2: 0)
  end

  def test_vercmp_alias
    # vercmp is the pacman-style comparator: a > b => 1, a == b => 0, a < b => -1
    assert_equal 1, Ssot::Lib::Common.vercmp('2.0.0', '1.0.0')
    assert_equal 0, Ssot::Lib::Common.vercmp('1.0.0', '1.0.0')
    assert_equal -1, Ssot::Lib::Common.vercmp('1.0.0', '2.0.0')
  end
end
...
class TestFormatVersion < Minitest::Test
  ...
  def test_double_digit_pkgrel
    assert_equal '1.0.0-99', Ssot::Lib::Common.format_version(0, '1.0.0', 99)
  end
  ...
  def test_large_epoch
    assert_equal '10:3.0.0-2', Ssot::Lib::Common.format_version(10, '3.0.0', 2)
  end
end
...
class TestValidateOutputFilename < Minitest::Test
  ...
  def test_valid_underscore
    assert_silent { Ssot::Lib::Common.validate_output_filename('my_rule.md', 'memory') }
  end
  ...
  def test_rejects_empty_string
    assert_raises(RuntimeError) { Ssot::Lib::Common.validate_output_filename('', 'pkg') }
  end
  ...
  def test_rejects_double_slash
    assert_raises(RuntimeError) { Ssot::Lib::Common.validate_output_filename('a//b.md', 'pkg') }
  end
end
...
class TestValidateTargetDir < Minitest::Test
  ...
  def test_valid_nested_dir
    assert_silent { Ssot::Lib::Common.validate_target_dir('skills/my-bundle/', 'pkg') }
  end
  ...
  def test_rejects_absolute_path
    assert_raises(RuntimeError) { Ssot::Lib::Common.validate_target_dir('/absolute/path/', 'pkg') }
  end

  def test_rejects_dotdot_only
    assert_raises(RuntimeError) { Ssot::Lib::Common.validate_target_dir('..', 'pkg') }
  end
end
...
class TestExpandUserPath < Minitest::Test
  ...
  def test_empty_string
    assert_equal '', Ssot::Lib::Common.expand_user_path('')
  end
...
class TestStripFrontmatter < Minitest::Test
  ...
  def test_empty_content_returns_empty
    assert_equal '', Ssot::Lib::Common.strip_frontmatter('')
  end
