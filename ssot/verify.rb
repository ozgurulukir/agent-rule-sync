...
# ssot/verify.rb — Verify index vs disk state
# Compares SSoT index (what should be installed) against actual files on disk.
...
PLATFORM_REGISTRY = Ssot::Lib::Common.load_platform_registry

def main
  platform_arg = ARGV.first

  unless Ssot::Lib::Common::INDEX_YAML_PATH.exist?
    abort "index.yaml not found. Run `ruby ssot/build.rb` first."
  end
  ...
  platforms_to_verify.each do |platform_id|
    platform_cfg = Ssot::Lib::Common.platform_config(platform_id, PLATFORM_REGISTRY)
    unless platform_cfg
...
def resolve_base_path(platform_cfg)
  project_root = Ssot::Lib::Common.project_root_for(platform_cfg, nil)
  if project_root
    return project_root
  end
...
def verify_package(platform_id, platform_cfg, pkgname, pkgdata, inst, base_path)
  ...
  installed_path = Ssot::Install.resolve_install_path_for_target(platform_cfg, target, base_path, base_path)
...
def scan_orphans(platform_id, platform_cfg, base_path, packages)
  ...
  packages.each do |pkgname, pkgdata|
    (pkgdata[:installed] || []).each do |inst|
      ...
      target = pkgdata[:targets]&.find { |t| t[:platform] == platform_id }
      next unless target
      p = Ssot::Install.resolve_install_path_for_target(platform_cfg, target, base_path, base_path)
      expected_top << p.to_s
    end
  end
