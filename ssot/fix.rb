...
# ssot/fix.rb — Fix drift between SSoT index and disk
# Runs verify, then repairs missing/checksum errors via reinstall.
# Usage: ruby ssot/fix.rb [platform] [--dry-run] [--auto]
...
def main
  ...
  platforms.each do |platform_id|
    ...
    cmd_out = `ruby ssot/verify.rb #{platform_id} 2>&1`
    verify_ok = $?.success?
    ...
    if has_drift
      if dry_run
        ...
        fix_result = system("ruby", "ssot/install.rb", platform_id)
        if fix_result
  ...
  if fixed_anything
    puts "\n✅ Fix applied. Run `ruby ssot/verify.rb` to confirm."
  else
    puts "\nℹ No fixes applied."
  end
end
...
def find_broken_packages(platform_id, index)
  platform_cfg = Ssot::Lib::Common.platform_config(platform_id, REGISTRY)
  ...
  (index[:packages] || {}).each do |pkgname, pkgdata|
    ...
    is_broken = if format_type == 'skill-bundle'
                  ...
                  !Ssot::Lib::Common::BUILD_DIR.join(platform_id, inst[:output]).exist?
                else
...
def resolve_install_path(platform_cfg, target, base_path)
  if target
    Ssot::Install.resolve_install_path_for_target(platform_cfg, target, base_path, base_path)
  else
    base_path
  end
end
...
def all_platforms_from_index
  index_path = Ssot::Lib::Common::INDEX_YAML_PATH
