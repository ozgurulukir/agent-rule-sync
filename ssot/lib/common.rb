    ...
    module Common
      ...
      def check_prerequisites(platform_cfg)
        ...
        Array(prereqs[:versions]).each do |tool, version_req|
          ...
          log_debug("  #{tool} version requirement: #{version_req} (not enforced)")
        end
      ...
      def load_pkgbuild(pkgdir)
        ...
        data = YAML.safe_load(raw, permitted_classes: [Symbol], symbolize_names: true)
      ...
      def cached_fetch_url(url, expected_sha256)
        ...
        log_debug("URL cached: #{url} → #{actual_sha256[0..7]}")
