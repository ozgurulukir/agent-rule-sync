...
# Run pkgver_func if defined. Returns true if successful or no pkgver_func.
...
def fetch_url(url_str, expected_sha256 = nil, max_redirects: Rulepack::Config.max_redirects)
  ...
  max_redirects.times do
    response = Net::HTTP.start(uri.host, uri.port, use_ssl: uri.scheme == 'https',
                                                   read_timeout: Rulepack::Config.read_timeout) do |http|
    ...
    when Net::HTTPSuccess
      ...
      if expected_sha256 && expected_sha256 != computed
        raise "SHA256 mismatch for #{url_str}: " \
              "expected #{expected_sha256}, got #{computed}. " \
              "Update the sha256 field in your PKGBUILD to: #{computed}"
      end
