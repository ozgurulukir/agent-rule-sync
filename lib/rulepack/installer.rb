...
module Rulepack
  module Install
    ...
    def run(platform_id, options = {})
      ...
      $LOG_LEVEL = verbose_mode ? :debug : Rulepack::Config.log_level
      ...
      unless Rulepack::Common::BUILD_INDEX_PATH.exist?
        Rulepack::Common.log_error "Build index not found at #{Rulepack::Common::BUILD_INDEX_PATH}. Run `ruby lib/rulepack/build.rb` first."
        exit 1
      end
    ...
    def install_all(options = {})
      ...
      Rulepack::Common.log "🚀 Installing ALL platforms (#{platforms.size} platforms)#{dry_run ? ' (dry-run)' : ''}"
    ...
    def should_install_or_upgrade?(pkgname, pkgdata, platform_id, index, force_mode, dry_run, quiet)
      ...
      case cmp
      when 0
        Rulepack::Common.log "  ↺ #{pkgname} already installed (#{Rulepack::Common.format_version(pkgdata[:epoch], pkgdata[:pkgver], pkgdata[:pkgrel])})" unless quiet
        false
      when 1
        Rulepack::Common.log "  🔄 Upgrading #{pkgname} #{Rulepack::Common.format_version(existing[:epoch], existing[:version], existing[:pkgrel])} → #{Rulepack::Common.format_version(pkgdata[:epoch], pkgdata[:pkgver], pkgdata[:pkgrel])}" unless quiet
    ...
    def handle_downgrade(pkgname, pkgdata, existing, force_mode, dry_run, quiet)
      if force_mode
        Rulepack::Common.log_warn "Downgrade forced for #{pkgname}: #{Rulepack::Common.format_version(existing[:epoch], existing[:version], existing[:pkgrel])} → #{Rulepack::Common.format_version(pkgdata[:epoch], pkgdata[:pkgver], pkgdata[:pkgrel])}" unless quiet
        uninstall_single_package_from_index!(index, pkgname, platform_id) unless dry_run
        true
      else
        Rulepack::Common.log_error "Downgrade detected for #{pkgname}: installed #{Rulepack::Common.format_version(existing[:epoch], existing[:version], existing[:pkgrel])}, candidate #{Rulepack::Common.format_version(pkgdata[:epoch], pkgdata[:pkgver], pkgdata[:pkgrel])}" unless quiet
    ...
    def verify_skill_bundle(installed_path, pkgname)
      ...
      begin
        ...
        if mismatches.empty?
          Rulepack::Common.log "  ✓ Skill-bundle manifest: #{manifest['sub_skills'].size} sub-skill(s), #{total_files} file(s) verified"
          nil
        else
    ...
    def install_skill_bundle(pkgname, pkgdata, target, platform_cfg, base_path,
      ...
      if dry_run
        selected.each { |ss| Rulepack::Common.log "    [DRY-RUN] Would copy sub-skill: #{ss['path']} → #{dest_dir.join(ss['path'])}" unless quiet }
      else
    ...
    def warn_large_bundle(build_src_dir, sub_skills)
      ...
      Rulepack::Common.log_warn "  ⚠ Large bundle: #{sub_count} sub-skills. Use --select <names> to install only specific ones." if sub_count > 50
    ...
    def select_sub_skills(sub_skills, select_list, pkgname)
      if select_list && !select_list.empty?
        ...
        if selected.empty?
          Rulepack::Common.log_warn "  ⚠ No matching sub-skills for --select #{select_list.join(',')} in #{pkgname}, skipping"
          return nil
        end
        selected
    ...
    def aggregate_vendor_skills(platform_id, platform_cfg, base_path)
      ...
      if $?.success?
        ...
        vendor_file = Rulepack::Common::BUILD_DIR.join(platform_id, 'skills', 'vendor', "#{platform_id}.md")
