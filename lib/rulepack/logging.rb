...
module Rulepack
  module Common
    ...
    def log(msg, level: :info, log_file: nil)
      ...
      puts line if level_order[level] <= level_order[Rulepack::Common.log_level]
    ...
    def time(operation_name)
      ...
      log("⏱  #{format('%.3f', elapsed)}s — #{operation_name}", log_file: nil) if Rulepack::Common.show_timing
      result
    end
  end
end
