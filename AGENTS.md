...
- **Platform registry is memoized per root**: `Rulepack::Platforms.load(root)` caches in a hash keyed by the expanded root path, so sandbox tests with their own registry get their own cache slot. Tests that mutate the *repo* registry must call `Rulepack::Platforms.clear_cache!` (or the `Common.clear_platform_registry_cache!` delegator) before re-reading. Global path override setters no longer exist — scope via `Rulepack::Common.with_paths(...)` or pass `paths:` to backend entry points (`Fix.run`, `Outdated.run`, `Bump.run`).
...
- **Minimal inline technical debt**: the scoped Paths/UI contexts in `common.rb` are a documented strangler scaffold — deep layers still read `Common.build_dir` / `Common.ui`; candidate 2 (typed build→install contract) is expected to collapse them. Most improvement work is tracked externally in `docs/improvement-plan/OPEN-ITEMS.md`.
