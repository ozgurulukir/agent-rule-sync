...
- `cli/commands.rb` — the dispatch table: PACMAN_ALIASES + one fully-declared row per command (backend/method, `call:`/`args:` for scalar-API rows, `defaults:`/`positional: :target` for positional remapping, `max_positional:`/`usage:`, `group:`/`synopsis:`/`description:`/`help_lines:` for help). Adding a command is a table row.
- `cli/runner.rb` — the CLI runner: single CliParser parse → alias remap → table dispatch → render → exit code. Global exit-code rule: **success → 0, partial/failure → 1** (drift, some-platforms-failed, outdated-found, bump-with-changes all exit 1).
- `platform_scanner.rb` — discovers rulepack-managed and manually installed items on disk.

Library files under `lib/rulepack/` are pure modules — no CLI runner blocks, no `$rulepack_exit_code` global. The CLI (`bin/rulepack`) is the single entry point; dispatch/rendering live in `lib/rulepack/cli/`. Backends return `Result` objects and narrate via `Emitter` events — never raw `puts`. E2E/integration tests copy `bin/` + `lib/` + `data/` into sandboxes and drive `bin/rulepack` subprocesses; the runner itself is covered in-process (`test/test_cli_runner.rb`).
...
All Result-producing commands support `--format text|json|yaml|jsonl` (build, install, uninstall, verify, check, fix, outdated, audit, bump, query/list/show/search/platforms, status, catalog, remote, lock, init-hooks). `jsonl` is a stream format (events + final `:result` line). Exit codes are uniform: **0 success, 1 partial/failure** (see `docs/agents/USAGE.md`).
...
- **CLI spine (2026-09-16, completed 2026-09-19)**: `lib/rulepack/cli/commands.rb` is the real dispatch table — **every** command is a fully-declared row (backend/method, `call: :args` for scalar-API rows, `defaults:`/`positional: :target` for positional remapping, `max_positional:`, `group:`/`synopsis:`/`description:` for help). `lib/rulepack/cli/runner.rb` parses once, executes rows, renders once, applies one exit-code rule (success 0, partial/failure 1); there is no local-command ladder and no Integer-return path. `Rulepack::Result#view` declares which TextRenderer renders the data in text mode — nothing is inferred from data shape; `view` is excluded from `to_h` so the json/yaml/jsonl envelope stays `{status, data, errors, messages}`. Help text is generated by `cli/help.rb` from the table (byte-identical to the former heredoc); adding a command is one row. The former runner-local commands are backends: `status.rb`, `build_catalog.rb`, `remote.rb`, `lock.rb`, `init_hooks.rb` (scoped `Paths`-anchored), plus the `build_all.rb` composite (Build → Aggregate) shared with `Bump.invoke_build`. `bump` parses through CliParser (`--apply`); no `raw_argv` rows remain. In-process runner coverage lives in `test/test_cli_runner.rb`.
- **Index store ownership (2026-09-19)**: `InstalledIndex` and `BuildIndex` are the only readers/writers of `data/index.yaml` and `build/index.yaml`; every access resolves through the scoped `Paths` — the repo-anchored `Common::BUILD_DIR`/`BUILD_INDEX_PATH`/`INDEX_YAML_PATH`/`LOG_PATH` constants are deleted, so there is no scope-blind path left to bypass a sandbox with (the `lock` command's lockfile is deliberately working-directory-anchored). `InstalledIndex.load` always migrates (SchemaMigration + `InstalledRecord.migrate_legacy!`); callers never migrate by hand. Neither store memoizes — callers mutate the loaded hash in place. `Fix.run`→`Install.run` passes the options hash positionally (the Ruby 4 keyword-collision crash is fixed and pinned by in-process sandbox tests in `test/test_fix.rb`).
