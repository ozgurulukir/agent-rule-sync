...
- **Minimal inline technical debt (updated 2026-09-16)**: the installed-record schema is now owned by `InstalledRecord` and disk-state verdicts by `InstalledState.check` (the three divergent verify/brokenness copies are gone); Fix's write-before-reinstall choreography is deleted in favor of `Install.run(force_packages:)`. Remaining known items: `UI.default=` is still load-bearing for tests (`test/helper.rb` sets a global Null UI — it can be removed only once tests inject `ui:` everywhere); `transform.rb` defines a native `Common.strip_frontmatter` duplicating `Path.strip_frontmatter`; `wire_renderer`'s renderer lifecycle is not re-entrant; `LocalCatalog` honors `paths:` in only one branch. Most improvement work is tracked externally in `docs/improvement-plan/OPEN-ITEMS.md`.
